...
if($this->StartResultCache(false, md5($APPLICATION->GetCurPage())))
{
	...
	else
	{
		...
		while($arSection = $rsSections->GetNext())
		{
			...
			//если мы в текущей секции или в параметрах задано выбирать все
			if((strpos($APPLICATION->GetCurPage(), $arSection["~SECTION_PAGE_URL"]) !== false)
				|| (strpos($APPLICATION->GetCurPage(), $arSection["~SECTION_PAGE_URL"]) === false
					&& $arParams["INCLUDE_ALL_ELEMENTS"] == "Y"))
			{
				//добавляем элементы
				$elementFilter = array(
					"IBLOCK_ID" => $arParams["IBLOCK_ID"],
